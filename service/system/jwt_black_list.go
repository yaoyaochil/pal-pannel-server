...
func (jwtService *JwtService) JwtInBlacklist(jwtList system.JwtBlacklist) (err error) {
	err = global.PalDB.Create(&jwtList).Error
	if err != nil {
		return
	}
	global.PalCache.SetDefault(jwtList.Jwt, struct{}{})
	return
}
...
func LoadAll() {
	var data []string
	err := global.PalDB.Model(&system.JwtBlacklist{}).Select("jwt").Find(&data).Error
	if err != nil {
		global.PalLog.Error("加载数据库jwt黑名单失败!", zap.Error(err))
		return
	}
	for i := 0; i < len(data); i++ {
		global.PalCache.SetDefault(data[i], struct{}{})
